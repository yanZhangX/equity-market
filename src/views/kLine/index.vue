<template>
  <div id="kLine">
    <NavBar title="比特币" fixed leftIcon="arrow-left" @clickLeft="goBack"/>
    <div class="overview">
        <span class="main-data view-item">6239.517</span>
        <div class="view-item">
            <p class="view-desc">开盘</p>
            <p class="vuew-num">6239.517</p>
        </div>
        <div class="view-item">
            <p class="view-desc">最低</p>
            <p class="vuew-num">6239.517</p>
        </div>
        <div class="view-item">
            <p class="view-desc">最高</p>
            <p class="vuew-num">6239.517</p>
        </div>
    </div>
    <div class="tabs-wrap">
        <switchCom v-model="isOn" text="K线|走势"></switchCom>
        <md-tab-bar
            v-model="current"
            :items="items"
            :maxLength="5"
    ...
    <div class="fixed-wrap">
        <div class="hold-btn">
            <img src="@/assets/icons/mixin/icon_coin.png" alt="">
            <span class="btn-desc">持仓</span>
        </div>
        <div class="up-btn">
            <img src="@/assets/icons/mixin/icon_buyUp.png" alt="">
            <span class="btn-desc">买涨</span>
        </div>
        <div class="fall-btn">
            <img src="@/assets/icons/mixin/icon_buyFall.png" alt="">
            <span class="btn-desc">买跌</span>
        </div>
    </div>
  </div>
</template>
...
import SwitchCom from "@/components/switch/index"
...
export default {
  ...
  methods: {
    ...
    calculateMA(dayCount) {
        var result = [];
        for (var i = 0, len = this.resData.values.length; i < len; i++) {
            if (i < dayCount) {
                result.push('-');
                continue;
            }
    ...
    calculateSA() {
        ...
        for (var i = 0, len = this.resData.values.length; i < len; i++) {
            if (i > 0) {
                var k = Math.abs(this.resData.values[i][3] - this.resData.values[i][2]) / this.resData.values[i - 1][1] * 100;
    ...
    calculateUD() {
        ...
        for (var i = 0, len = this.resData.values.length; i < len; i++) {
            if (i > 0) {
                var k = (this.resData.values[i][1] - this.resData.values[i - 1][1]) / this.resData.values[i - 1][1] * 100;
                result.push(k.toFixed(2));
            }
        }
        return result;
    ...
    setEchartOption () {
      ...
      this.echartsOption.xAxis.forEach((val,key)=>{
